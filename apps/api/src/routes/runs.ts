...
import { nowUAE, shiftOf, formatDurationMs, splitDayNight, shiftDayBounds, splitShiftDay, correctBucket, effRunHalf, effDownDay } from '@shared/timeCalculations';
...
export const runsRoutes = new Hono<AuthContext>()
  .get('/', zValidator('query', querySchema), async (c) => {
    ...
    if (machine_status) query = query.where(eq(machines.status, machine_status as any));
  ...
  .post('/start', zValidator('json', runStartSchema), async (c) => {
    ...
    // @ts-ignore - drizzle type inference
    const machine = await db.select().from(machines).where(eq(machines.id, data.machine_id)).limit(1) as any;
  ...
  .post('/stop/:rid', async (c) => {
    ...
    // @ts-ignore - drizzle type inference
    const run = await db.select().from(productionRuns).where(eq(productionRuns.id, rid)).limit(1) as any;
    ...
    const machine = await db.select().from(machines).where(eq(machines.id, run[0].machine_id)).limit(1) as any;
    if (machine.length) {
      await db.update(machines)
    // @ts-ignore - drizzle type inference
    ...
    // @ts-ignore - drizzle type inference
    const updated = await db.select().from(productionRuns).where(eq(productionRuns.id, rid)).limit(1) as any;
  ...
  .get('/export', zValidator('query', querySchema), async (c) => {
    ...
    const runs = await query.orderBy(desc(productionRuns.started_at)).all() as any;
