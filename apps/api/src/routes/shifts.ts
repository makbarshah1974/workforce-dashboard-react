...
export const shiftRoutes = new Hono()
  .get('/', zValidator('query', z.object({
    ...
    const total = await db.select({ count: count() }).from(shifts).where(is_active !== undefined ? eq(shifts.is_active, is_active === 'true') : undefined);
  ...
  .get('/assignments', zValidator('query', querySchema), async (c) => {
    ...
    if (status) conditions.push(eq(shiftAssignments.status, status));
  ...
  .get('/assignments/calendar', zValidator('query', z.object({
    ...
    const assignments = await db.select({
      ...
      .orderBy(shiftAssignments.date, shifts.name);
  ...
  .get('/:id', async (c) => {
    const db = createDb(c.env);
    const shift = await db.select().from(shifts).where(eq(shifts.id, c.req.param('id'))).limit(1);
  ...
  .patch('/:id', zValidator('json', shiftSchema.partial()), async (c) => {
    const data = c.req.valid('json');
    const db = createDb(c.env);
    const [shift] = await db.update(shifts).set({ ...data, updated_at: new Date() }).where(eq(shifts.id, c.req.param('id'))).returning();
    if (!shift) return c.json({ error: 'Shift not found' }, 404);
    return c.json({ data: shift });
  })
  .delete('/:id', async (c) => {
    const db = createDb(c.env);
    const [shift] = await db.delete(shifts).where(eq(shifts.id, c.req.param('id'))).returning();
    if (!shift) return c.json({ error: 'Shift not found' }, 404);
  ...
  .post('/assignments', zValidator('json', assignmentSchema), async (c) => {
    ...
    const existing = await db.select().from(shiftAssignments).where(and(eq(shiftAssignments.worker_id, data.worker_id), eq(shiftAssignments.date, data.date))).limit(1);
  ...
  .post('/assignments/bulk', zValidator('json', z.object({
    ...
    for (const a of assignments) {
      const existing = await db.select().from(shiftAssignments).where(and(eq(shiftAssignments.worker_id, a.worker_id), eq(shiftAssignments.date, a.date))).limit(1);
      if (!existing.length) {
        const [created] = await db.insert(shiftAssignments).values(a).returning();
  ...
  .patch('/assignments/:id', zValidator('json', assignmentSchema.partial()), async (c) => {
    const data = c.req.valid('json');
    const db = createDb(c.env);
    const [assignment] = await db.update(shiftAssignments).set({ ...data, updated_at: new Date() }).where(eq(shiftAssignments.id, c.req.param('id'))).returning();
    if (!assignment) return c.json({ error: 'Assignment not found' }, 404);
    return c.json({ data: assignment });
  })
  .delete('/assignments/:id', async (c) => {
    const db = createDb(c.env);
    const [assignment] = await db.delete(shiftAssignments).where(eq(shiftAssignments.id, c.req.param('id'))).returning();
    if (!assignment) return c.json({ error: 'Assignment not found' }, 404);
